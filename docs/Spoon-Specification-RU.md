...
Spoon — нативный графический SVN-клиент для macOS с быстрым и ясным интерфейсом, учитывающим централизованную модель и особенности Subversion.
...
7. **Самостоятельный дизайн.** Spoon использует собственную кодовую базу, визуальную систему и принципы взаимодействия.
...
| CHG-005 | Изменения файловой системы и SVN-метаданных запускают один отложенный status refresh. |
| CHG-006 | Spoon периодически выполняет remote-aware status; пользователь также может запросить его вручную. |
...
- Изменение `.svn` внешним SVN-клиентом считается сигналом перечитать status, но Spoon не читает служебную базу напрямую.
...
- Пока приложение активно, локальный status перепроверяется каждые 15 секунд, remote status — каждую минуту и при возврате в приложение.
...
- App icon и бренд Spoon используют только собственные визуальные ресурсы.
...
5. Spoon использует собственную кодовую базу, визуальную систему и UX.
